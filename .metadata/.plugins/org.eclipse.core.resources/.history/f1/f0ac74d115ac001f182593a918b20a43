...
public class SimularVentas {
	...
	public static double simularProductoCantidad (ArrayList<String> productos, ArrayList<Integer> cantidades) {
		...
		for(int i = 0; i < compra; i++) {
			...
			if(ides.contains(id + 1)) {
				if(baseDatos.get(id).getTipo() == 2) {
					...
					int cantidad = generarNumeroAleatorioTipo2(1.0, 4.0);
					if(cantidad <= baseDatos.get(id).getStock()) {
						actualizarStock(cantidad, "" + baseDatos.get(id).getNumero());
						productos.add("" + baseDatos.get(id).getNumero());
	...
	public static int generarNumeroAleatorioTipo2(double min, double max) {
